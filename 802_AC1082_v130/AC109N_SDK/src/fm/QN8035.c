...
const u8 qn8035_vol[] AT (TABLE_CODE)=
{
...
//长天线配置:	
u8 _xdata qnd_PreNoiseFloor,qnd_NoiseFloor;

//短天线配置:	
//u8 _xdata qnd_PreNoiseFloor = 35,qnd_NoiseFloor = 35;
...
*/
/*----------------------------------------------------------------------------*/
#define	qn8035_rssi  1     /*0 松*/    /* 5 紧*/
#define    qn8035_if2 0x03   /*0x05 松*/     /* 0x02 紧*/
#define    qn8035_snr_th1  0xc8    /*0xc7 松*/   /* 0xc9 紧*/
#define	qn8035_snr_th2  0xc5
#define	qn8035_pilot_cca  1  /*1 立体声*/	   /* 0 紧*/
...
void qn8035_mute1(bool On) AT(QN0835_CODE)
{
...
Return Value:
	None
**********************************************************************/
void QND_TuneToCH(u16 ch) AT(QN0835_CODE)               //手动设置频点
{
	u8 reg;
...
//#endif	
	...
	///for(i=0;i<4;i++)
	///{
		///if(QND_ReadReg(0x03) < 60) 	
		{
			///delay_n10ms(1); 		
			///QNF_SetRegBit(0x00,0x04,0x04);   ///mo
			///break;
		}
		///delay_n10ms(2);
		///QNF_SetRegBit(0x00,0x04,0x00);           ///st
	///}
...
*/
/*----------------------------------------------------------------------------*/
__root bool QND_RXValidCH(u16 freq) AT(QN0835_CODE)
{
	u8 regValue;
	u8 timeOut;
	u8 isValidChannelFlag;
//	u8 snr,snc,temp1,temp2;
//#if PILOT_CCA
	...
	if(qn8035_pilot_cca)
	{
		for(readCnt=10;readCnt>0;readCnt--)
		{
			...
			if(stereoCount >= 3) return 1;
                ...
                delay_n10ms(2);
		if((QN_IIC_read(SNR))> qn8075_snr) return 1;  // 25
	}	
	else
		return 1;
//#endif
...
*/
/*----------------------------------------------------------------------------*/
__root bool QN8035_Read_ID(void) AT(QN0835_CODE)
{
	u8 cChipID;
