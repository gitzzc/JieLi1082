...
const u8 rda5807sp[] AT(RDA5807_TABLE_CODE)= {
    ...
    RDA5807_RSSI | 0x80, 0xBF, //05H:       //0xB0 音量为0
    0x80, 0x00,
    0x5F, 0x1A, //07H
    0x5e, 0xc6,
    0x00, 0x00,
...
*/
/*----------------------------------------------------------------------------*/
void rda5807p_set_vol(u8 vol) AT(RDA5807_CODE)
{
    vol = vol/2; 
    if(vol > 15)
        vol = 15;
    ...
    rda5807_dat[3] &= ~ BIT(4);  //03H.4=0,disable tune
    if(vol == 0)
    {
        rda5807_dat[0] &= ~(1<<6);//MUTE ENABLE
    }
    else
    {
        rda5807_dat[0] |= (1<<6)|(1<<7);
        rda5807_dat[1] |= (1<<0);
    }
    rda5807_write(8);
}
