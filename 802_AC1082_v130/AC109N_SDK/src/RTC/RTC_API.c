...
*/
/*----------------------------------------------------------------------------*/
bool leapyear(u16 year) AT(RTC_CODE)
{
...
*/
/*----------------------------------------------------------------------------*/
u8 nowmonth(u8 nmonth,u16 nyear) AT(RTC_CODE)
{
    if (leapyear(nyear))
...
/*Everyday alarm*/
   ...
   if ((curr_alarm.bHour < curr_time.bHour)||((curr_alarm.bHour == curr_time.bHour)&&(curr_alarm.bMin <= curr_time.bMin)))
    ...
    //my_printf("--Next alarm: %02x\n",curr_alarm.bSw);
    //my_printf("--Next alarm: %u/%u/%u %u:%u\n",alm_sec.dYear,(u16)alm_sec.bMonth,(u16)alm_sec.bDay,(u16)alm_sec.bHour,(u16)alm_sec.bMin);
}
...
void IRTC_init(void) AT(RTC_CODE)
{
    ...
    /*---------Power down reset RTC sfr*/
    reg = read_RTC_reg(READ_SR0);
    //my_printf("IRTC reg 0 : %02x\n", reg);
    if (reg & BIT(7))
    {
        reg &= ~BIT(7);
        reset_RTC_time();
        reset_RTC_RAM();
        //my_puts("1-Power Down\n");
    }
#ifdef RTC_ALARM_EN
    /*---------Alarm info*/
    curr_alarm.bSw = 0;		
    ...
    /*----------Alarm initial--------------*/
    //reg = read_RTC_reg(READ_SR1);
    //my_printf("IRTC reg 1 : %02x\n", reg);
...
void RTC_info_init(void)  AT(RTC_CODE)
{
    ...
    read_RTC((RTC_TIME *)(&curr_time));
    RTC_overflow_check();
    //my_printf("Date :%u/%u/%u  ", curr_time.dYear, (u16)curr_time.bMonth, (u16)curr_time.bDay);
...
#endif
    /*Recover keytone*/
    // my_printf("ALM_vol_info.rtc_volr = %u\n",ALM_vol_info.rtc_volr);
     //my_printf("ALM_vol_info.rtc_voll = %u\n",ALM_vol_info.rtc_voll);
