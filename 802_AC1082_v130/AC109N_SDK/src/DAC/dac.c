...
_near_func void set_key_tone(void) AT(COMMON_CODE)
{
	...
	{
		KV_VLM = 8;//((digital_vol_tab[main_vol_L]>>8) + (digital_vol_tab[main_vol_R]>>8))/2;	  //按键音的音量
	}
	KV_CNT = 0;
	KV_CNT = 20;					  //按键音重复次数
}
...
_near_func void dac_check(void) AT(COMMON_CODE)
{
    ...
    if (fade_l_in || fade_r_in)
    {
        ...
        {
            temp_l = (30 - DECODE_VOL_LIMIT_L);  //31
            temp_r = (30 - DECODE_VOL_LIMIT_R);  //31
        }
...
{
    DAC_setting();
    main_vol_L = MAX_MAIN_VOL_L;//MAX_MAIN_VOL_L;//get_memory(MEM_VOL_L);
   // main_vol_L = ((main_vol_L > MAX_MAIN_VOL_L) || (main_vol_L == 0)) ? 15 : main_vol_L;
    main_vol_R = MAX_MAIN_VOL_R;//MAX_MAIN_VOL_R;//get_memory(MEM_VOL_R);
